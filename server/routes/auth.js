...
async function storeOTP({ target, target_type, otp_code, otp_type }) {
  await pool.query(
    'DELETE FROM otps WHERE target = $1 AND otp_type = $2',
    [target, otp_type]
  );
...
async function verifyOTPCode({ target, otp_code, otp_type }) {
  const result = await pool.query(
    `SELECT * FROM otps WHERE target=$1 AND otp_code=$2 AND otp_type=$3 AND used=FALSE AND expires_at > NOW()`,
...
router.post('/send-otp', async (req, res) => {
  const { target, target_type, otp_type } = req.body;
  if (!target || !target_type || !otp_type)
    return res.status(400).json({ error: 'Missing required fields' });

  try {
    if (otp_type === 'signup' && target_type === 'email') {
      const ex = await pool.query('SELECT id FROM users WHERE email=$1', [target]);
    ...
    if (otp_type === 'reset') {
      const col = target_type === 'email' ? 'email' : 'phone';
...
router.post('/complete-signup', async (req, res) => {
  const { name, email, password, phone, otp_code, otp_target, otp_target_type } = req.body;
...
router.post('/reset-password', async (req, res) => {
  const { target, target_type, otp_code, new_password } = req.body;
  ...
  try {
    ...
    const col = target_type === 'email' ? 'email' : 'phone';
    const result = await pool.query(`UPDATE users SET password_hash=$1 WHERE ${col}=$2 RETURNING id`, [hash, target]);
...
router.post('/find-account', async (req, res) => {
  ...
  try {
    const result = await pool.query('SELECT email FROM users WHERE phone=$1', [phone]);
